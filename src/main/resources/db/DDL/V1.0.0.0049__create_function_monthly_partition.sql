CREATE OR REPLACE PROCEDURE mdm.create_monthly_partition(p_year INT,
p_schema_name VARCHAR,p_table_name VARCHAR,p_disable_autovacuum boolean)
LANGUAGE plpgsql    
AS $$
DECLARE
...
v_partition_end_dt :=v_partition_start_dt + interval '1 Month' * 1;
...
exit when i>500; -- just to make sure loop doesn't go to infinite loop due to any  condition failure
